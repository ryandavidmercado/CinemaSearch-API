...
function generateConfigFromPrefix(obj, prefix) {
  const slicePoint = prefix.length + 1;
  const getSuffix = (key) => key.slice(slicePoint);

  let output = {};
  for (let key in obj) {
    if (key.startsWith(`${prefix}.`)) {
      const suffix = getSuffix(key);
      output[key] = [prefix, null, suffix];
    }
  }
...
module.exports = { reduceProperties, generateConfigFromPrefix };
